...
public class DetalleVentaRepoImpl implements IDetalleVentaRepo {
	...
	public List<ReporteVentas> buscarReporte(LocalDateTime fechaVenta, String categoria, Integer cantidad) {
		TypedQuery<ReporteVentas> myTypedQuery = this.e
				.createQuery(
						"SELECT NEW com.uce.p4.repository.modelo.ReporteVentas(p.codigoBarras, p.nombre, d.cantidad, d.precioUnitario, d.subtotal) FROM DetalleVenta d  JOIN  d.venta v JOIN  d.producto p WHERE v.fecha >= :fecha AND p.categoria = :categoria AND d.cantidad > :cantidad ",
						ReporteVentas.class)
